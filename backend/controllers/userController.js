...
const registerUser = async (req, res) => {
    ...
    try {
        ...
        if(exists) {
            return res.json({success:false, message:"user esiste gia'.."});
        }
        ...
        if(password.length < 6) {
            return res.json({success:false, message:"password con almeno 6 caratteri"});
        }
        ...
        const token = createToken(user._id);
        res.json({success:true, message:"utente creato", token});
